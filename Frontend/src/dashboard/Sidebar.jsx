    ...
    <>
      ...
      <div className={`w-64 h-full shadow-lg fixed top-0 left-0 bg-gray-50 transition-transform duration-300 transform sm:translate-x-0 ${show ? "translate-x-0" : "-translate-x-full"}`}>
        ...
        <div className='text-center'>
          <img className='h-24 w-24 rounded-full mx-auto mb-2' src={profile?.user?.photo?.url} alt="" />
        ...
        <ul className='space-y-6 mx-4'>
          ...
          <button onClick={() => handleComponents("Create Blog")} className='w-full px-4 py-2 bg-blue-500 rounded-lg hover:bg-blue-700 transition duration-300'>CREATE BLOG</button>
